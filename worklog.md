...
---
Task ID: home-suds-mood-crisis
Agent: Koda
Task: Доработка HomePage по senior health-tech UX: SUDS-чек-ин вокруг практик, ежедневный чек-ин настроения, кризисная ссылка, CTA «Продолжить путь», переименование «узоры мышления» → «ошибки мышления»

Work Log:
- **SUDS-чек-ин вокруг быстрых практик**: src/features/checkin/store.ts — новый модуль. `useSudsCheckInStore` (persist 'zabotapsy-suds-checkin', IndexedDB) хранит pending-чек-ин { sudsBefore, startedAt, practice }. На главной — опциональный раскрывающийся слайдер «Отметить тревогу до практики» (никогда не блокирует SOS-доступ: без отметки тап по практике работает как раньше). После возвращения с практики (≥2 мин, <24 ч) показывается карточка «Как тревога после практики „{name}“?» со слайдером; при отправке считается дельта: положительная → texts.journal.anxietyDecreased, иначе поддерживающий текст
- **Ежедневный чек-ин настроения**: тот же модуль — `useMoodNudgeStore` (persist 'zabotapsy-mood-nudge') хранит lastDismissedDate. Карточка «Как твой день?» с 5 эмодзи-кнопками (MOODS теперь экспортируется из @/features/mood) пишет напрямую в useMoodStore.addEntry + toast. Показывается только премиум-пользователям (журнал настроения — premium-фича; не заманиваем бесплатных в пейволл — тёмный паттерн), максимум 1 раз в день, «Не сегодня» отклоняет до следующего дня. Web Push отложен: без push-сервера честного планирования уведомлений нет, локальный nudge на главной — рабочая альтернатива
- **Кризисная ссылка**: под SOS-плитками тихая ссылка «Нужна помощь специалиста?» (LifeBuoy) → Radix Dialog с tel:-ссылками: 112, МЧС +7 (495) 989-50-50, детский телефон доверия 8-800-2000-122 + дисклеймер «не заменяет профессиональную помощь»
- **CTA «Продолжить путь»**: главная кнопка внизу HomePage ведёт в journal-new, текст texts.home.ctaMain; подсказка в карточке дневника обновлена соответственно (A/B-вариант B выбран пользователем)
- **Переименование «узоры мышления» → «ошибки мышления»** по всему приложению: texts.ts (thoughtPatternTitle/Help), achievements.ts (2 описания), DataExport.tsx (CSV-заголовок), articles/data.ts (4 места), комментарий JournalWizard.tsx, название теста schemas.test.ts
- src/shared/constants/texts.ts — новая секция `home`: sos, pre/post-чек-ин, кризисный диалог, mood-nudge, CTA
- src/shared/ui/RegisterPersistHydration.tsx — зарегистрированы 2 новых стора в реестре hydration
- HomePage.tsx — все новые блоки hydration-safe (useHydrated('zabotapsy-suds-checkin'/'zabotapsy-mood')), SSR-безопасны, в рамках Z*-дизайн-системы, без новых зависимостей
- Stage Summary: 4 сценария реализованы (SUDS-дельта, retention-петля настроения, кризисная ссылка, CTA), терминология «ошибки мышления» унифицирована. Проверка: tsc ✓, eslint ✓, 40/40 unit-тестов ✓, next build ✓
